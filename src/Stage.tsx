...
export class Stage extends StageBase<InitStateType, ChatStateType, MessageStateType, ConfigType> {
  ...
  constructor(data: InitialData<InitStateType, ChatStateType, MessageStateType, ConfigType>) {
    ...
    this.myInternalState['counters'] = this.myInternalState['counters'] || { total: 0 };
  ...
  async beforePrompt(userMessage: Message): Promise<Partial<StageResponse<ChatStateType, MessageStateType>>> {
    ...
    this.myInternalState.counters.total += 1;
    const total = this.myInternalState.counters.total;

    if (total < stageThresholds.green) this.myInternalState.stage = 'white';
    else if (total < stageThresholds.purple) this.myInternalState.stage = 'green';
    else if (total < stageThresholds.golden) this.myInternalState.stage = 'purple';
    else if (total < stageThresholds.red) this.myInternalState.stage = 'golden';
    else this.myInternalState.stage = 'red';
    ...
    let canRevealSecret = ['purple','golden','red'].includes(currentStage);
    let modifiedMessage: string | null = null;
    if (!canRevealSecret && content.toLowerCase().includes("ask secret")) {
      modifiedMessage = "Hmm, I can't tell you that yet...";
    }