...
export class Stage extends StageBase<any, any, any, any> {
  ...
  async beforePrompt(userMessage: Message): Promise<Partial<StageResponse<any, any>>> {
    ...
    // Update stage based on cumulative messages
    const total = this.state.totalMessages;
    if (total >= cumulativeThresholds.red) this.state.stage = "red";
    else if (total >= cumulativeThresholds.golden) this.state.stage = "golden";
    else if (total >= cumulativeThresholds.purple) this.state.stage = "purple";
    else if (total >= cumulativeThresholds.green) this.state.stage = "green";
    else this.state.stage = "white";

    // Update affection based on keywords
  ...
  render(): ReactElement {
    return <div></div>; // Empty, no Hello World
  }
}